...
class InlinePagination:
    ...
    def get_page_info_buttons(self, cur_page: int) -> List[InlineKeyboardButton]:
        ...
        if cur_page == all_pages:
            ...
            back_button = InlineKeyboardButton(
                callback_data=self.back_prefix + str(cur_page - 1), text="⏪ Orqaga"
            )
    ...
    def get_page_keyboard(self, cur_page: Union[int, str]):
        ...
        self.kb.row(InlineKeyboardButton(text="◀️ Menyuga qaytish", callback_data="back"))
        return self.kb
...
class InlinePagination2:
    ...
    def get_page_info_buttons(self, cur_page: int, collection_id: int) -> List[InlineKeyboardButton]:
        ...
        if cur_page == all_pages:
            back_button = InlineKeyboardButton(
                callback_data= f"{self.back_prefix}{collection_id}_" + str(cur_page - 1), text="⏪ Orqaga"
            )
            return [back_button, page_button]
        elif cur_page == 1:
            next_button = InlineKeyboardButton(
                callback_data=f"{self.next_prefix}{collection_id}_" + str(cur_page + 1), text="⏩ Oldinga"
            )
            return [page_button, next_button]
        else:
            next_button = InlineKeyboardButton(
    ...
    def get_page_keyboard(self, cur_page: Union[int, str], collection_id: int, fave_status: bool):
        ...
        self.kb.row(InlineKeyboardButton(text="😍 Onlayn ko‘rish", url=f'https://bot.kinozzz.ru/play/?id={film_id}'))
        if fave_status:
            self.kb.row(InlineKeyboardButton(text="❌ Sevimlilardan o‘chirish", callback_data=f'del_favorite|{film_id}'))
        else:
            self.kb.row(InlineKeyboardButton(text="➕ Sevimlilarga qo‘shish", callback_data=f'add_favorite|{film_id}'))
        self.kb.row(*info_page_buttons)
        return self.kb
...
class FavoritesPagination:
    ...
    def get_page_info_buttons(self, cur_page: int) -> List[InlineKeyboardButton]:
        ...
        if cur_page == all_pages:
            back_button = InlineKeyboardButton(
                callback_data= f"{self.back_prefix}" + str(cur_page - 1), text="⏪ Orqaga"
            )
            return [back_button, page_button]
        elif cur_page == 1:
            next_button = InlineKeyboardButton(
                callback_data=f"{self.next_prefix}" + str(cur_page + 1), text="⏩ Oldinga"
            )
            return [page_button, next_button]
        else:
            next_button = InlineKeyboardButton(
